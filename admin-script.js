...
function autoCheckoutMissed() {
    ...
    records.forEach(r => {
        if (r.checkIn && !r.checkOut && r.date !== today) {
            ...
            // Calculate overtime capped at 3 hours
            const empSettings = JSON.parse(localStorage.getItem('employeeSettings') || '{}');
            const s = empSettings[r.name] || {};
            const overtimeHour = s.overtimeHour || 17;
            const [h, m] = r.checkOut.split(':').map(Number);
            if (h >= overtimeHour) {
                const rawMinutes = (h - overtimeHour) * 60 + m;
                r.overtimeMinutes = Math.min(rawMinutes, 180); // Cap at 3 hours
            } else {
                r.overtimeMinutes = 0;
            }
...
function syncFromSheets() {
    fetch(`${APPS_SCRIPT_URL}?type=all`)
        ...
        .then(data => {
            ...
            // Only refresh summary and calendar, not the settings form
            renderCalendar();
            loadAttendance();
        })
        .catch(e => console.log('Sheets sync failed, using local data:', e));
}
...
function setupMonthSelector() {
    ...
    if (monthSelect.options.length > 0) return; // Already set up
    const now = new Date();
            ...
            <div class="employee-setting-card">
                ...
                <div id="emp_settings_${key}">
                    ...
                    <div id="view_${key}">
                        ...
                        <button class="edit-emp-btn" onclick="enableEdit('${key}', '${name}')">✏️ Edit</button>
                    </div>
                    ...
                    <div id="edit_${key}" style="display:none;">
                        ...
                        <div class="setting-grid">
                            ...
                            <div class="setting-row"><label>Monthly Salary (Rp)</label><input type="number" id="sal_${key}" value="${s.salary}" placeholder="0"></div>
                            <div class="setting-row"><label>Daily Bonus (Rp)</label><input type="number" id="bon_${key}" value="${s.dailyBonus}" placeholder="0"></div>
                            <div class="setting-row"><label>Overtime starts at (24h)</label><input type="number" id="oth_${key}" value="${s.overtimeHour || 17}" placeholder="17" min="0" max="23"></div>
                            <div class="setting-row"><label>Overtime/hour (Rp)</label><input type="number" id="ot_${key}" value="${s.overtimeBonus}" placeholder="0"></div>
                            <div class="setting-row"><label>Shift Pay/day (Rp)</label><input type="number" id="sh_${key}" value="${s.shiftBonus}" placeholder="0"></div>
                            <div class="setting-row"><label>Paid Leave (days/month)</label><input type="number" id="pl_${key}" value="${s.paidLeave || 0}" placeholder="0"></div>
                        </div>
                        <div class="settings-section-label monthly-label">Section B — ${MONTH_NAMES[month]} ${year}</div>
                        <div class="setting-grid">
                            <div class="setting-row"><label>Additional Bonus (Rp)</label><input type="number" id="add_${key}" value="${ms.additionalBonus}" placeholder="0"></div>
                            <div class="setting-row"><label>Advance Pay (Rp)</label><input type="number" id="adv_${key}" value="${ms.advancePay}" placeholder="0"></div>
                        </div>
...
function saveOneEmployee(name, key) {
    ...
    fetch(APPS_SCRIPT_URL, { method: 'POST', headers: { 'Content-Type': 'text/plain' }, body: JSON.stringify({ action: 'saveSettings', settings: employeeSettings }) }).catch(() => {});
    fetch(APPS_SCRIPT_URL, { method: 'POST', headers: { 'Content-Type': 'text/plain' }, body: JSON.stringify({ action: 'saveMonthlySettings', monthlySettings: JSON.parse(localStorage.getItem('monthlySettings') || '{}') }) }).catch(() => {});
...
function loadAttendance() {
    ...
    // Only re-render settings section labels (month/year), not the whole form
    updateSettingsSectionLabels(month, year);
}

function updateSettingsSectionLabels(month, year) {
    // Update only the Section B labels without re-rendering the whole form
    document.querySelectorAll('.monthly-label').forEach(el => {
        el.textContent = `Section B — Monthly Settings (${MONTH_NAMES[month]} ${year} only)`;
    });
}
    const holidays = JSON.parse(localStorage.getItem('holidays') || '[]');
    const daysInMonth = new Date(year, month + 1, 0).getDate();
