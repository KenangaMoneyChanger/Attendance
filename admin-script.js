...
function loadFromSheetsAndRender() {
    fetch(`${APPS_SCRIPT_URL}?type=all`)
        .then(r => r.json())
        .then(data => {
            // Always overwrite with Sheets data (source of truth)
            if (Array.isArray(data.attendance)) localStorage.setItem('attendance', JSON.stringify(data.attendance));
...
function autoCheckoutMissed() {
    ...
    records.forEach(r => {
        ...
        // Parse the record date (DD/MM/YYYY)
        const parts = r.date.split('/');
        if (parts.length !== 3) return;
        const recordDate = new Date(parseInt(parts[2]), parseInt(parts[1]) - 1, parseInt(parts[0]));
        
        // Only auto-checkout if the record is from a PREVIOUS day
        if (recordDate >= todayMidnight) return;

        r.checkOut = '23:59';
        r.overtimeMinutes = 0; // Don't count overtime for auto-checkout - admin can manually set if needed
        changed = true;
