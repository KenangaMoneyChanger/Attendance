...
// EMPLOYEES defined in script.js
        ...
        container.innerHTML += `
            <div class="employee-setting-card">
                ...
                <div id="emp_settings_${key}">
                    <div class="settings-section-label">Section A — Permanent Settings (applies from current month forward)</div>
...
function renderSummary(records, month, year) {
    ...
    EMPLOYEES.forEach(name => {
        const s = employeeSettings[name] || { salary: 0, dailyBonus: 0, overtimeBonus: 0, shiftBonus: 0 };
        const ms = getMonthlySettings(name, month, year);
