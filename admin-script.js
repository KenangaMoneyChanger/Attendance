...
// Consistent DD/MM/YYYY formatting regardless of device locale
...
function showAdminPanel() {
    setupMonthSelector();
    // Always load from Sheets on page load
    loadFromSheetsAndRender();
    // Auto-refresh every 2 minutes
    setInterval(loadFromSheetsAndRender, 120000);
}

function loadFromSheetsAndRender() {
    fetch(`${APPS_SCRIPT_URL}?type=all`)
        .then(r => r.json())
        .then(data => {
            console.log('Sheets response:', JSON.stringify(data).substring(0, 500));
            console.log('Attendance records count:', Array.isArray(data.attendance) ? data.attendance.length : 'not an array');
            if (Array.isArray(data.attendance) && data.attendance.length > 0) {
                console.log('First record:', JSON.stringify(data.attendance[0]));
            }
            // Normalize attendance dates in case Sheets returned them in unexpected format
            if (Array.isArray(data.attendance)) {
                data.attendance = data.attendance.map(r => {
                    if (r.date && !/^\d{2}\/\d{2}\/\d{4}$/.test(r.date)) {
                        // Try to parse and reformat
                        const d = new Date(r.date);
                        if (!isNaN(d)) {
                            r.date = String(d.getDate()).padStart(2,'0') + '/' +
            ...
            // Always overwrite with Sheets data (source of truth), only if data is non-empty
            if (Array.isArray(data.attendance) && data.attendance.length > 0) localStorage.setItem('attendance', JSON.stringify(data.attendance));
            if (data.settings && Object.keys(data.settings).length > 0) {
                console.log('Settings from Sheets:', JSON.stringify(data.settings).substring(0, 300));
                employeeSettings = data.settings;
                localStorage.setItem('employeeSettings', JSON.stringify(data.settings));
            } else {
                console.log('No settings in Sheets response, keeping local');
                employeeSettings = JSON.parse(localStorage.getItem('employeeSettings') || '{}');
            }
            if (data.monthlySettings) localStorage.setItem('monthlySettings', JSON.stringify(data.monthlySettings));
            if (Array.isArray(data.employeeList) && data.employeeList.length > 0) localStorage.setItem('employeeList', JSON.stringify(data.employeeList));
            if (Array.isArray(data.holidays)) localStorage.setItem('holidays', JSON.stringify(data.holidays));
        ...
        .catch(e => {
            console.log('Sheets sync failed, using local:', e);
            employeeSettings = JSON.parse(localStorage.getItem('employeeSettings') || '{}');
            renderEmployeeSettings();
            renderCalendar();
            loadAttendance();
...
function setupMonthSelector() {
    ...
    // Restore last selected month/year if coming back from detail page
    const savedMonth = sessionStorage.getItem('lastMonth');
    const savedYear = sessionStorage.getItem('lastYear');
    const activeMonth = savedMonth !== null ? parseInt(savedMonth) : now.getMonth();
    const activeYear = savedYear !== null ? parseInt(savedYear) : now.getFullYear();

    MONTH_NAMES.forEach((m, i) => {
        const opt = document.createElement('option');
        opt.value = i;
        opt.textContent = m;
        if (i === activeMonth) opt.selected = true;
...
function renderCalendar() {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        const isToday = dateStr === formatDateDMY(new Date());
...
function saveHolidaysToSheets() {
    const holidays = JSON.parse(localStorage.getItem('holidays') || '[]');
    // Ensure all holidays are plain strings in DD/MM/YYYY format
    const cleanHolidays = holidays.filter(d => typeof d === 'string' && /^\d{2}\/\d{2}\/\d{4}$/.test(d));
    syncPost({ action: 'saveHolidays', holidays: cleanHolidays });
    alert(`${cleanHolidays.length} holidays saved to Google Sheets!`);
}
...
function toggleHoliday(dateStr, isWeekend) {
    ...
    // Don't auto-sync on every click - use Save Holidays button instead
    renderCalendar();
    loadAttendance();
}
...
function renderEmployeeSettings() {
    ...
    employees.forEach(name => {
        const s = employeeSettings[name] || { position: '', salary: 0, dailyBonus: 0, overtimeBonus: 0, overtimeHour: 19, shiftBonus: 0, paidLeave: 0 };
                ...
                <div id="emp_settings_${key}">
                    <div id="view_${key}">
                        ...
                        <div class="readonly-grid">
                            ...
                            <div class="readonly-row"><span>Overtime Starts</span><strong>19:00</strong></div>
                            <div class="readonly-row"><span>Overtime 7–8PM/hour</span><strong>Rp 10.000</strong></div>
                            <div class="readonly-row"><span>Overtime 8PM+/hour</span><strong>Rp 20.000</strong></div>
                    ...
                    <div id="edit_${key}" style="display:none;">
                        ...
                        <div class="setting-grid">
                            <div class="setting-row"><label>Position</label><input type="text" id="pos_${key}" value="${s.position}" placeholder="e.g. Teller"></div>
                            ...
                            <div class="setting-row"><label>Overtime Starts</label><input type="text" value="19:00" disabled style="background:#f0f0f0;color:#999;"></div>
                            <div class="setting-row"><label>Overtime 7–8PM/hour</label><input type="text" value="Rp 10.000" disabled style="background:#f0f0f0;color:#999;"></div>
                            <div class="setting-row"><label>Overtime 8PM+/hour</label><input type="text" value="Rp 20.000" disabled style="background:#f0f0f0;color:#999;"></div>
...
function saveOneEmployee(name, key) {
    ...
    // Re-sync from Sheets after saving
    setTimeout(loadFromSheetsAndRender, 2000);
...
function loadAttendance() {
    ...
    console.log('loadAttendance: total records in localStorage:', records.length);
    console.log('loadAttendance: filtering for month', month, 'year', year);

    const filtered = records.filter(r => {
        const parts = r.date ? r.date.split('/') : [];
        if (parts.length !== 3) {
            console.log('Bad date format:', r.date, r);
            return false;
        }
    ...
    console.log('loadAttendance: filtered records:', filtered.length);
...
function renderSummary(records, month, year) {
    ...
    getEmployees().forEach(name => {
        ...
        const presentDays = empRecords.length; // all check-ins this month
        const regularDays = empRecords.filter(r => !r.isShift).length;
        const shiftDays = empRecords.filter(r => r.isShift).length;
        const absent = Math.max(0, workDays - regularDays); // only weekday absences
        ...
        let totalTier1Minutes = 0;
        let totalTier2Minutes = 0;
        empRecords.forEach(r => {
            if (r.overtimeMinutes) totalOvertimeMinutes += r.overtimeMinutes;
            // Fall back: if tier data missing but overtimeMinutes exists, treat first 60 as tier1, rest as tier2
            if (r.overtimeTier1Minutes || r.overtimeTier2Minutes) {
                totalTier1Minutes += r.overtimeTier1Minutes || 0;
                totalTier2Minutes += r.overtimeTier2Minutes || 0;
            } else if (r.overtimeMinutes) {
                totalTier1Minutes += Math.min(r.overtimeMinutes, 60);
                totalTier2Minutes += Math.max(0, r.overtimeMinutes - 60);
            }
        });

        // Tiered overtime: 19-20 = Rp10.000/hr, 20+ = Rp20.000/hr
        const overtimeBonus = Math.round((totalTier1Minutes / 60) * 10000) + Math.round((totalTier2Minutes / 60) * 20000);
        ...
        // Collect for overtime summary
        overtimeSummaryRows.push({ name, totalOvertimeMinutes, totalTier1Minutes, totalTier2Minutes, overtimeBonus });
    ...
    html += `
        <div style="margin-top:24px;">
            <h3 style="font-size:15px;color:#003366;margin-bottom:10px;">Overtime Summary — ${MONTH_NAMES[month]} ${year}</h3>
            <div class="table-scroll">
            <table>
                <thead>
                    <tr>
    ...
    if (overtimeSummaryRows.length === 0) {
        html += `<tr><td colspan="5" style="text-align:center;color:#999;">No overtime this month</td></tr>`;
    } else {
        let totalOTPay = 0;
        overtimeSummaryRows.forEach(r => {
            const fmt = n => n.toLocaleString('id-ID');
            const t1h = (r.totalTier1Minutes / 60).toFixed(1);
            const t2h = (r.totalTier2Minutes / 60).toFixed(1);
            const totalH = (r.totalOvertimeMinutes / 60).toFixed(1);
            totalOTPay += r.overtimeBonus;
            html += `
                <tr>
                    ...
                    <td>${t2h} hrs</td>
                    <td>${totalH} hrs</td>
                    <td>Rp ${fmt(r.overtimeBonus)}</td>
                </tr>
            `;
        });
        html += `
            <tr style="background:#f0f4ff;">
                <td colspan="4"><strong>Total Overtime Pay</strong></td>
                <td><strong>Rp ${totalOTPay.toLocaleString('id-ID')}</strong></td>
            </tr>
        `;
    }
...
function openDetail(name) {
    ...
    // Save current month/year so we can restore when coming back
