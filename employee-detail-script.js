    ...
    sorted.forEach(r => {
        ...
        html += `
            <tr>
                ...
                <td>${r.checkOut ? formatTime(r.checkOut) : `<button class="checkout-admin-btn" onclick="adminCheckout('${r.date}', '${name}')">Check Out</button>`}</td>
                <td>${type}</td>
                <td>${ot}</td>
            </tr>
        `;
...
function adminCheckout(date, name) {
    ...
    if (h >= overtimeHour) {
        records[idx].overtimeMinutes = (h - overtimeHour) * 60 + m;
    } else {
