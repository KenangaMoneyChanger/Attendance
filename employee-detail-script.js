    ...
    document.getElementById('monthlySummaryDetail').innerHTML = `
        ...
        <table class="earnings-table">
            ...
            <tbody>
                ...
                <tr>
                    <td>Overtime 7–8PM (${tier1Hours.toFixed(1)}h × Rp 10.000)
                        <small style="display:block;color:#999;font-size:11px;">19:00–20:00</small>
                    </td>
                    <td>Rp ${fmt(Math.round(tier1Hours * 10000))}</td>
                </tr>
                <tr>
                    <td>Overtime 8PM+ (${tier2Hours.toFixed(1)}h × Rp 20.000)
                        <small style="display:block;color:#999;font-size:11px;">20:00 and above</small>
                    </td>
...
function adminCheckout(date, name) {
    ...
    const overtimeHour = s.overtimeHour || 19;
    const [h, m] = time.split(':').map(Number);
    if (h >= overtimeHour) {
        records[idx].overtimeMinutes = Math.min((h - overtimeHour) * 60 + m, 180); // Cap at 3 hours
    } else {
