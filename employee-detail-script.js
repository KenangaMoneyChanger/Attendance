...
function init() {
    ...
    renderAttendanceLog(filtered, name);
}
...
function renderMonthlySummary(records, s, ms, month, year, holidays) {
    ...
    const tier1Hours = totalTier1Minutes / 60;
    const tier2Hours = totalTier2Minutes / 60;
    const overtimeBonus = Math.round(tier1Hours * 10000) + Math.round(tier2Hours * 20000);
    ...
    document.getElementById('monthlySummaryDetail').innerHTML = `
        <div class="summary-cards">
            ...
            <div class="summary-card blue">
                <span>Overtime</span>
                <strong>${(totalOvertimeMinutes / 60).toFixed(1)}h</strong>
            </div>
        </div>
        ...
        <table class="earnings-table">
            ...
            <tbody>
                ...
                <tr>
                    <td>Overtime 6–7PM (${tier1Hours.toFixed(1)}h × Rp 10.000)
                        <small style="display:block;color:#999;font-size:11px;">18:00–19:00</small>
                    </td>
                    <td>Rp ${fmt(Math.round(tier1Hours * 10000))}</td>
                </tr>
                <tr>
                    <td>Overtime 7PM+ (${tier2Hours.toFixed(1)}h × Rp 20.000)
                        <small style="display:block;color:#999;font-size:11px;">19:00 and above</small>
                    </td>
                    <td>Rp ${fmt(Math.round(tier2Hours * 20000))}</td>
                </tr>
...
function renderAttendanceCalendar(records, month, year, holidays) {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
        ...
        if (isWeekend) {
            ...
            if (record) timeInfo = `<span class="cal-time">${formatTime(record.checkIn)}${record.checkOut && record.checkOut !== '' ? '→'+formatTime(record.checkOut) : ''}</span>`;
        } else if (record) {
            cls += record.isShift ? ' shift' : ' present';
            timeInfo = `<span class="cal-time">${formatTime(record.checkIn)}${record.checkOut && record.checkOut !== '' ? '<br>→'+formatTime(record.checkOut) : ''}</span>`;
        } else {
...
function formatTime(val) {
    ...
    // If it's a full ISO timestamp, extract time in WIB (UTC+7)
    try {
...
function renderAttendanceLog(records, name) {
    if (records.length === 0) {
    ...
    sorted.forEach(r => {
        ...
        html += `
            <tr>
                <td>${r.date}</td>
                <td>${dayName}</td>
                <td class="present">${formatTime(r.checkIn)}</td>
                <td>${(r.checkOut && r.checkOut !== '') ? formatTime(r.checkOut) : `<button class="checkout-admin-btn" onclick="adminCheckout('${r.date}', '${name}')">Check Out</button>`}</td>
                <td>${type}</td>
                <td>${ot}</td>
            </tr>
        `;
...
function adminCheckout(date, name) {
    const time = prompt(`Enter checkout time for ${name} on ${date} (HH:MM):`, '17:00');
    if (!time || !/^\d{2}:\d{2}$/.test(time)) {
        alert('Invalid time format. Use HH:MM (e.g. 17:00)');
        return;
    }

    const records = JSON.parse(localStorage.getItem('attendance') || '[]');
    const idx = records.findIndex(r => r.name === name && r.date === date);
    if (idx < 0) { alert('Record not found.'); return; }

    records[idx].checkOut = time;

    // Calculate overtime using employee's overtime hour setting
    const employeeSettings = JSON.parse(localStorage.getItem('employeeSettings') || '{}');
    const s = employeeSettings[name] || {};
    const overtimeHour = s.overtimeHour || 18;
    const [h, m] = time.split(':').map(Number);
    if (h >= overtimeHour) {
        const rawMinutes = Math.min((h - overtimeHour) * 60 + m, 240); // Cap at 4 hours
        records[idx].overtimeMinutes = rawMinutes;
        records[idx].overtimeTier1Minutes = Math.min(rawMinutes, 60);  // 18-19 = Rp10.000/hr
        records[idx].overtimeTier2Minutes = Math.max(0, rawMinutes - 60); // 19+ = Rp20.000/hr
    } else {
        records[idx].overtimeMinutes = 0;
        records[idx].overtimeTier1Minutes = 0;
        records[idx].overtimeTier2Minutes = 0;
    }

    localStorage.setItem('attendance', JSON.stringify(records));

    // Sync to Sheets using no-cors (fire and forget)
    ...
    const payload = JSON.stringify({
        action: 'checkout', name, date, time,
        timestamp: new Date().toISOString(),
        overtimeMinutes: records[idx].overtimeMinutes
    });
    
    // Use a hidden form to POST (bypasses CORS redirect issue)
