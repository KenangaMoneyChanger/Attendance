...
async function awardCoins(name) {
    try {
        ...
        // Format today as DD/MM/YYYY explicitly — never rely on locale
        const now = new Date();
        const today = formatDateDMY(now);
        ...
        // Last expected workday before today, zeroed to midnight
        const lastExpectedWorkday = getPreviousWorkday(now, holidays);
        lastExpectedWorkday.setHours(0, 0, 0, 0);

        // Streak continues only if they checked in on the last expected workday
        const lastCheckInDate = current.lastCheckIn ? parseDate(current.lastCheckIn) : null;
        const streakContinues = lastCheckInDate &&
            lastCheckInDate.getTime() >= lastExpectedWorkday.getTime();
        ...
        // Await the save so stale data isn't read on a quick retry
