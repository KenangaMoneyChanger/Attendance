...
const OFFICE3_LAT = -6.24848434732291;
const OFFICE3_LNG = 106.80121379178331;
const OFFICE3_RADIUS_METERS = 100;
...
function init() {
    ...
    // Check location immediately — don't wait for anything
    checkLocation();
    // Load employee list in background
    loadEmployeeListFromSheets().catch(() => {});
}
...
function checkLocation() {
    ...
    navigator.geolocation.getCurrentPosition(
        (position) => {
            ...
            const distance3 = getDistanceMeters(
                userLocation.latitude, userLocation.longitude,
                OFFICE3_LAT, OFFICE3_LNG
            );
            const distance = Math.min(distance1, distance2, distance3);
            const nearestOffice = distance1 <= distance2 && distance1 <= distance3 ? 'Office 1'
                                : distance2 <= distance3 ? 'Office 2' : 'Office 3';

            const withinOffice = distance1 <= OFFICE_RADIUS_METERS
                              || distance2 <= OFFICE2_RADIUS_METERS
                              || distance3 <= OFFICE3_RADIUS_METERS;
...
function showCheckInForm() {
    ...
    // Populate dropdown — if list not loaded yet, retry after Sheets fetch completes
    ...
    function populateDropdown() {
        ...
        // List not ready yet — retry in 500ms
